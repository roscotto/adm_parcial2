        ...
        <v-row>
          <v-row>
            <v-col cols="12" class=" div-h3 p-2 mt-5 d-flex">
              ...
              <h3 color="textoGris">Recetas Guardadas</h3>
            </v-col>
            <v-card
              class="mx-auto my-12"
              max-width="374"
          ...
          <v-container class="my-6">
            <v-row v-if="libroDeCocina.length == 0">
              <h2 class="text-center">No tenes recetas guardadas... Chusmea nuestras recetas</h2>
            </v-row>
          </v-container>
        </v-row>
      </v-container>
...
<script>
  export default {
    ...
    mounted: function(){
      if (localStorage.getItem('misRecetasGuardadas') != undefined){
        this.libroDeCocina = JSON.parse(localStorage.getItem('misRecetasGuardadas'))
      }
    },
    filters:{
